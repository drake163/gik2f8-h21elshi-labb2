...
function renderTask({ id, title, description, dueDate, completed }) {
  ...
  */
  ...
  if (completed == true) {
    ...
    html += `
      <div class="flex items-center">
        <h3 class="mb-3 flex-1 text-xl font-bold text-slate-600 uppercase">${title}</h3>
        <div>
          <span>${dueDate}</span>
          <button onclick="deleteTask(${id})" class="inline-block bg-slate-400 text-xs text-black border border-white px-3 py-1 rounded-md ml-2">Ta bort</button>
        </div>
      </div>`;
  }
  ...
  if (completed == true) {
    html +=
      `
    <div id="id${id}" class="box-border my-2 mr-2 h-8 w-8 border-4 border-black" onclick="markDone(${id}, ${completed})">
    ...
    html +=
      `
      <div id="id${id}" class="box-border my-2 mr-2 h-8 w-8 border-4 border-lime-500" onclick="markDone(${id}, ${completed})">
        <div id="child${id}" class="m-1 h-4 w-4 border-3 bg-lime-500"></div>
      </div>
    `;
  }
